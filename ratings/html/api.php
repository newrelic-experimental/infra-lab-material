...
class RatingsAPI extends API {
    ...
    private function _dbConnect() {
        $mysql_host = getenv('MYSQL_HOST') ? getenv('MYSQL_HOST') : 'mysql';
        $dsn = 'mysql:host=' . $mysql_host . ';dbname=ratings;charset=utf8mb4';
        $opt = array(
    ...
    private function _checkSku($sku) {
        $catalogue_host = getenv('CATALOGUE_HOST') ? getenv('CATALOGUE_HOST') : 'catalogue';
        $url = 'http://' . $catalogue_host . ':8080/';
        $url = $url . 'product/' . $sku;
